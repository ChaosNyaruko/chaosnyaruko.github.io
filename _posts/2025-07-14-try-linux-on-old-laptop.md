...
- 资源占用小到我穿越回20年前
  ...
  - TODO截图对比。我的电脑是2019款的MagicBook（8+512）的锐龙版，配置是比较低的，当时也只卖3000左右。之前用Windows的时候，一开机就70%的内存占用，几乎干什么都卡 + 发热 + 风扇狂转。安装完ArchLinux + 一些必要的服务（比如sshd/bluez/xorg-server/clash/...等）+ 轻量级桌面/窗口管理器（比如i3）+ 终端 + tmux，占用内存才**400MB**！！加一个Firefox播放B站视频，也才1G多一点。再次fxxk microsoft

- 安装过程出乎意料的顺利

  - 我是下载的完整镜像，并使用自带的archinstall脚本。
...
- Ghostty这个终端模拟机非常的Out of the Box，不是说其他的不好（我也用Alacritty/Kitty啥的），只是你如果想快速用起来的话，我挺推荐这个的，它还自带Nerd font
...
- 休眠机制，因为之前没注意，swap分区是1G的，不知道对休眠有什么影响
